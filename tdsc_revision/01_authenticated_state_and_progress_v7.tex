...
transaction that is active in a committed state and whose candidate
live-ancestor sets along the subsequent committed fragment sequence are
contained in a fixed finite set.  Section~\ref{sec:discussion-scope} discusses
this condition.
...
A LocalOrder is admissible only if its signature, epoch, sender, and sequence are
valid,
...
dictionary $\mathsf{Done}_r$, which maps each identifier to its committed sequence
...
We establish agreement, graph-materialization equivalence, safe-fragment
validity, batch-order-fairness, retention safety, recovery, and conditional
progress.  Batch-order-fairness follows because prefix-complete evidence forms
a strict edge from every still-live required predecessor to the transaction
being finalized, and safe down-closure places that predecessor in the same or
an earlier batch.  Complete proofs appear in
Appendix~\ref{app:autig-security-details}.
...
Algorithm~\ref{alg:apply-evidence}, adding every newly active vertex, and
re-evaluating each pair in $\mathsf{TouchedPairs}_r$ whose endpoints lie in
$\widehat A_r$ yields
$\widehat G_{\mathrm{utig},r}=\widehat G_r^\star$.  After committed
...
Algorithm~\ref{alg:verify-fragment-expanded} accepts $\mathcal F_r$, then
$F_r$ contains only application-valid, data-available transactions and
$\mathrm{set}(F_r)=K_r^{\mathsf{safe}}$.  Moreover, $\mathsf{Part}_r$ is
exactly the SCC partition of
$\widehat G_r^\star[K_r^{\mathsf{safe}}]$, and its ranks and serialization
equal the public deterministic rules.
\end{theorem}
...
Let an unpruned execution and a soft-pruned execution start from the same
committed state $\Sigma_{r-1}$ and apply the same admissible evidence batch
$\mathcal L_r$.  Provided that the soft-pruned execution retains every
authoritative position and retains or deterministically reconstructs every
required live--live pair value, both executions derive the same
$\widehat G_r^\star$, $K_r^{\mathsf{safe}}$, $\mathsf{Part}_r$, serialized
$F_r$, and $\Sigma_r^{\mathsf{cand}}$.  Their structural witnesses, and hence
$\mathcal P_r$, $\mathcal F_r$, and $H_{\mathcal F_r}$, may differ.
\end{theorem}
...
positions and AUTIG live-data reference and evict all derived values incident
to $u$.  Relative to an execution retaining these entries, deletion changes
no future live--live pair value, candidate graph, safe closure, fairness-batch
assignment, or outstanding fairness obligation.
\end{theorem}
...
of its valid proposals by the hosting BFT.  Let $u$ be valid with
$u\in A_{r_0}$ for some committed sequence $r_0$.  Suppose that there exists a
finite set $B$ such that, for every committed fragment $\mathcal F_r$ with
$r>r_0$ and $u\in\mathsf{Live}_{r-1}$,
$\mathsf{Anc}_{\widehat G_r^\star}(u)\subseteq B$.  If every transaction that
appears in one of these ancestor sets is eventually received by all correct
replicas, then $u$ is eventually included in a committed safe closure.
\end{proposition}

Without a finite ancestor envelope, newly appearing Shaded ancestors may
...
$O(n(|\mathsf{Live}_r|+1))$ entries for positions, per-replica log state, and
live-data references, plus the selected representation of $\mathsf{Done}_r$;
transaction contents are excluded.  An optional aggregate-weight cache
occupies $O(|\mathsf{Live}_r|^2)$ counters.

Let $N_i$ and $K_i^-$ denote replica $i$'s newly admitted and previously
retained live positions.  Evidence application touches
$O(\sum_i(|N_i||K_i^-|+|N_i|^2))$ unordered pairs.  With cached pair weights,
incremental UTIG refresh costs
$O(|\mathsf{TouchedNodes}_r|+|\mathsf{TouchedPairs}_r|)$.  The leader stores
$O(|\widehat A_r|+|\widehat E_r^\star|)$ graph entries, and SCC decomposition,
condensation construction, and minimum-$\mathsf{cid}$ Kahn ordering take
$O(|\widehat A_r|+|\widehat E_r^\star|
  +|\widehat A_r|\log|\widehat A_r|)$ time.

A fragment contains $n-f$ signed LocalOrders with at most
$(n-f)\,lo\text{-}size$ identifiers, and a structural certificate of
$O(|F_r|+|V_{\mathsf{blk}}|)$ records, where
$|V_{\mathsf{blk}}|\le|\widehat A_r\setminus\mathrm{set}(F_r)|$.
Referenced transaction bytes are workload-dependent and excluded.

Structural verification performs
$\binom{|F_r|}{2}+(|\widehat A_r|-|F_r|)|F_r|
 +O(|V_{\mathsf{blk}}|)$ pair-value queries.  With $O(1)$ cached-weight
lookup, its cost is $O(|\widehat A_r||F_r|+|V_{\mathsf{blk}}|)$ and is
quadratic in $|\widehat A_r|$ in the worst case; direct reconstruction from
positions adds an $O(n)$ factor.  Complete fragment verification additionally
replays $\mathcal L_r$, checks $n-f+1$ signatures, and validates the
availability and application validity of newly admitted transactions.
...
Themis-style standard liveness could be pursued by composing AUTIG with an
additional bounded-domain or deferred-ordering mechanism.  Designing and
evaluating that composition is outside the scope of this work.  The protocol
analyzed here retains safe closure and establishes the stabilized-ancestry
progress property of Proposition~\ref{prop:stabilized-progress}.
...
each root is Shaded with depth zero, and each non-root $v$ stores a parent
$p(v)$ and depth satisfying $\widehat{\mathsf{EdgePred}}_r(p(v)\to v)$;
...
Starting from the same $\Sigma_{r-1}$ and applying the same $\mathcal L_r$,
deterministic evidence application yields identical $\widehat\Sigma_r$ and
$\widehat{\mathsf{View}}_r$.  Authoritative positions uniquely determine every
visibility count, state, active vertex, and directional live--live pair
weight.  Rematerializing any evicted cache therefore reconstructs the same
$\widehat G_r^\star$.

SCC decomposition, safe-closure marking, Kahn ordering, and intra-SCC
serialization are deterministic functions of this graph.  Consequently, both
executions derive the same $K_r^{\mathsf{safe}}$, $\mathsf{Part}_r$, serialized
$F_r$, and $\Sigma_r^{\mathsf{cand}}$ because $h_r$ binds
$(\widehat h_r,F_r,\mathsf{Part}_r)$ rather than the structural witness.  They
...
Every transaction that appears as a live ancestor after $r_0$ belongs to the
finite set $B$.  By eventual dissemination and admission, each such
transaction either finalizes earlier or, while it remains live, accumulates
positions from at least $n-f$ correct replicas and becomes Solid.  Because
only finitely many such identifiers can appear, after some finite sequence
every live ancestor of $u$ is Solid.  Hence $u\in K_r^{\mathsf{safe}}$; a
correct leader's maximal safe extraction includes $u$, and the hosting BFT
eventually commits that fragment.
\end{proof}
